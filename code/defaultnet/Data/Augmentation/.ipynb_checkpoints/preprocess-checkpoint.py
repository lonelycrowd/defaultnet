...
class RandomCrop(BaseMultiTransform):
    ...
    def __init__(self, jitter, crop_anno=False, intersection_threshold=0.001, fill_color=127):
        ...
        self.crop_modifier = CropModifier(float('Inf'), intersection_threshold)
    ...
    def _tf_anno(self, annos):
        ...
        if self.crop_anno:
            modify(annos, [self.crop_modifier])
        else:
...
def modify(boxes, modifier_fns):
    ...
    Warning:
        ...
        >>> import copy
        >>>
        >>> new_boxes = modify(copy.deepcopy(boxes), [modfier_fns, ...])
    """
    ...
    def __call__(self, box):
        ...
        if w <= 0 or h <= 0 or ratio:
            if self.discard_lost:
                ...
                box.lost = True
                if self.update_truncated and isinstance(box, Box) and box.truncated_fraction < 1:
                    if w <= 0 or h <= 0:
